...
ALTER TABLE `user` ADD FOREIGN KEY (`team_id`) REFERENCES `team`(`id`) ON UPDATE CASCADE ON DELETE CASCADE;

DROP TABLE IF EXISTS 	`student` ;
CREATE TABLE `student` (
	`id`					INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
	`fullName`				VARCHAR(30) NOT NULL,
	`gender`				VARCHAR(30) NOT NULL,-- ENUM('MALE', 'FEMALE', 'OTHER'),
	`phone_number`			VARCHAR(15) NOT NULL,--  UNIQUE,
	`email`					VARCHAR(50) NOT NULL UNIQUE,
    `birthday`				VARCHAR(50),-- DATETIME,
    `school`				VARCHAR(50),
	`address`				VARCHAR(50),
    `social_info`			VARCHAR(50),
    `target`				VARCHAR(50),-- ENUM('IT', 'NON_IT', 'TESTER'),
    `status`				VARCHAR(50),-- ENUM('INTERESTING', 'NON_INTERESTING', 'OTHER'),
    `history_transaction`	VARCHAR(50),-- NVARCHAR(100),
    `source`				VARCHAR(50),-- ENUM('ONLINE', 'OFFLINE', 'EVENT'),
    `team_id`				INT UNSIGNED,
...
-- INSERT INTO	`team` (`name`, `team_lead_id`) VALUES ('developer', 1);team
...
--  INSERT INTO `student` ( `full_name`, `gender`, `phone_number`, `email`, `birthday`, `school`, `address`, `social_info`, `target`, `status`, `history_transaction`, `source`, `created_date` ) 
